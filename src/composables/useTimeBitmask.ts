...
export function useTimeBitmask() {
  // 7天的数据，每天用一个 BigInt 表示 48 个半小时
  const weekState = ref<bigint[]>(new Array(7).fill(0n))

  // 从字符串数组导入
  const fromStringArray = (data: string[][]) => {
    const newState = new Array(7).fill(0n)
