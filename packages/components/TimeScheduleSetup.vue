    ...
    <div ref="calendar" class="schedule-calendar">
      ...
      <div class="table-wrap">
        <table class="schedule-calendar-table">
          <thead>
            <tr class="schedule-calendar-time-all">
              <!-- 空白占位列，用于显示周几标签 -->
          ...
          <tbody>
            <tr v-for="(dayLabel, index) in dayLabels" :key="index">
              ...
              <td
                ...
                :class="{
                  'schedule-calendar-selected': selectedJudgement(index, time),
                  'schedule-calendar-disabled': isDayTimeDisabled(index, time) || false,
                  'schedule-calendar-atom-time': true
                }"
                :data-day="index"
                :data-time="time"
...
function updateValue(newValue: number[][], options = { emitError: false }) {
  ...
  const newClonedValue: number[][] = JSON.parse(JSON.stringify(newValue))
